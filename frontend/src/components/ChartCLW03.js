...
const ChartCLW = memo(({title,subtitle,height,color,namespace,dimension_name,dimension_value,metric_name,stat_type,period,interval,metric_precision,format,current_metric_mode,font_color_value="#4595dd", pageId=1, itemsPerPage=2, groupByDimensionId = 0}) => {
    ...
    var refreshOnDemand = useRef(0);
    var pageIdLocal = useRef(1);
    ...
    if (pageId != pageIdLocal.current ){
        pageIdLocal.current = pageId;
        refreshOnDemand.current = 1;
        fetchMetrics();
    }
    ...
    function fetchMetrics(){
            ...
            var beginItem = ( ( pageIdLocal.current - 1) * itemsPerPage );
            var endItem = ( ( pageIdLocal.current - 1) * itemsPerPage ) + itemsPerPage;
            ...
            dimensionValues.forEach(function(item) {
                    ...
                    if ( queryId >= beginItem && queryId < endItem ) {
                            ...
                            dataQueries.push({
                                    ...
                                    Label: values[groupByDimensionId]
                            });
            ...
            Axios.get(`${configuration["apps-settings"]["api_url"]}/api/aws/clw/region/query/`,{
             params: queryclw
                    ...
                    if ( timestampMetric.current != data.data.MetricDataResults[0].Timestamps[0] || refreshOnDemand.current == 1) {
